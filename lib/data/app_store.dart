...
class AppStore extends ChangeNotifier {
  final List<Equipment> equipment = [
    ...
    Equipment(tag: 'P-2101B', name: 'Process Pump B', area: 'Unit', state: EquipmentState.running),
    Equipment(tag: 'E-2204', name: 'Process Exchanger', area: 'Unit', note: 'Monitor ΔP'),
    Equipment(tag: 'FV-2205', name: 'Flow Control Valve', area: 'Unit', state: EquipmentState.running, note: 'Operating watch item'),
  ];
  ...
  final List<OperatorTask> tasks = [
    OperatorTask(id: 't1', title: 'Follow up mechanical inspection', equipmentTag: 'P-2101A'),
  ];
  ...
  void addLog(String text, {String? tag}) {
    final clean = text.trim();
    if (clean.isEmpty) return;
    logs.insert(0, LogEntry(id: DateTime.now().microsecondsSinceEpoch.toString(), createdAt: DateTime.now(), text: clean, equipmentTag: tag));
    notifyListeners();
  }

  void setEquipmentState(Equipment item, EquipmentState state) {
    item.state = state;
    addLog('${item.tag} → ${state.name}', tag: item.tag);
    notifyListeners();
  }

  void addWatch(String title, String detail, {String? tag}) {
    watch.insert(0, WatchItem(id: DateTime.now().microsecondsSinceEpoch.toString(), title: title, detail: detail, equipmentTag: tag));
    notifyListeners();
  }
